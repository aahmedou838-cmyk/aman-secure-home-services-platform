...
import { User, Sparkles, ShieldCheck, RefreshCw, Info } from "lucide-react";
...
export default function CharacterSelectPage() {
  ...
  const handleRandomize = () => {
    const prefixes = ["مستكشف", "صقر", "رحالة", "عابر", "ظل", "حارس"];
    const randomPrefix = prefixes[Math.floor(Math.random() * prefixes.length)];
    const randomNum = Math.floor(1000 + Math.random() * 9000);
    setNickname(`${randomPrefix}_${randomNum}`);
    setSelectedIdx(Math.floor(Math.random() * AVATARS.length));
  };
      ...
      <div className="py-8 md:py-10 lg:py-12">
        <div className="max-w-3xl mx-auto space-y-12" dir="rtl">
          <div className="text-center space-y-4">
            ...
            {isGuest && (
              <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-aman-amber/10 text-aman-amber rounded-lg text-xs font-bold">
                <Info className="w-3 h-3" />
                <span>أنت في وضع الضيف. تقدمك مؤقت حتى تقوم بالتسجيل.</span>
              </div>
            )}
          </div>
          ...
          <div className="bg-card p-8 rounded-[2.5rem] border shadow-sm space-y-6 max-w-md mx-auto">
            <div className="space-y-2">
              <div className="flex justify-between items-center mb-1">
                <label className="text-sm font-bold text-muted-foreground">لقبك السري في المدينة</label>
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={handleRandomize}
                  className="h-8 px-2 text-aman-teal hover:bg-aman-teal/5 flex gap-1 items-center"
                >
